...
//   corrected_L = observed_L - offset_L - (vig[0]*r^2 + vig[1]*r^4 + vig[2]*r^6) - brdf*theta^2
...
struct RadiometricMatchCost
{
    ...
    RadiometricMatchCost(const float observed_a[3], const float observed_b[3], float r_a, float r_b, float theta_a,
                         float theta_b)
        : _r_a(r_a), _r_b(r_b), _theta_a(theta_a), _theta_b(theta_b)
    {
        for (int i = 0; i < 3; i++)
        {
    ...
    bool operator()(const T *lab_offset_a, const T *brdf_a, const T *vig_a, const T *lab_offset_b, const T *brdf_b,
                    const T *vig_b, T *residuals) const
    {
        T r2_a = T(_r_a * _r_a);
        ...
        for (int c = 0; c < 3; c++)
        {
            ...
            if (c == 0)
            {
                // L channel gets vignetting + BRDF correction
                corr_a -= vig_corr_a + brdf_corr_a;
                corr_b -= vig_corr_b + brdf_corr_b;
            }
