...
// Prior: penalize brightness slope toward zero
struct SlopePrior
{
    static constexpr int NUM_RESIDUALS = 2;
    static constexpr int NUM_PARAMETERS_1 = 2; // slope[2]
...
// Pairwise prior: penalize slope differences between overlapping cameras.
// Prevents slope oscillation where neighbors develop opposite slopes.
// Weight scaled by sqrt(num_correspondences) so well-connected pairs are constrained more.
struct SlopeSmoothnessCost
{
    static constexpr int NUM_RESIDUALS = 2;
    static constexpr int NUM_PARAMETERS_1 = 2; // slope_a
    static constexpr int NUM_PARAMETERS_2 = 2; // slope_b

    double _weight;

    explicit SlopeSmoothnessCost(double weight) : _weight(weight)
    {
    }

    template <typename T> bool operator()(const T *slope_a, const T *slope_b, T *residuals) const
    {
        residuals[0] = T(_weight) * (slope_a[0] - slope_b[0]);
        residuals[1] = T(_weight) * (slope_a[1] - slope_b[1]);
        return true;
    }
};

// Prior: penalize spatial gradient in LAB offsets.
// For each camera at normalized position (nx, ny), penalizes offset_L * nx and offset_L * ny.
// This resists a linear trend in brightness across the mosaic.
struct SpatialGradientPrior
{
    static constexpr int NUM_RESIDUALS = 6;    // L*nx, L*ny, a*nx, a*ny, b*nx, b*ny
    static constexpr int NUM_PARAMETERS_1 = 3; // lab_offset

    double _weight;
    float _norm_x; // camera position normalized to [-1, 1]
    float _norm_y;

    SpatialGradientPrior(double weight, float norm_x, float norm_y) : _weight(weight), _norm_x(norm_x), _norm_y(norm_y)
    {
    }
    ...
    template <typename T> bool operator()(const T *lab_offset, T *residuals) const
    {
        for (int c = 0; c < 3; c++)
        {
            residuals[c * 2 + 0] = T(_weight) * lab_offset[c] * T(_norm_x);
            residuals[c * 2 + 1] = T(_weight) * lab_offset[c] * T(_norm_y);
        }
        return true;
    }
};
