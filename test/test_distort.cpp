...
void verifyIsInverse(const Model1 &model1, const Model2 &model2, double eps = 1e-12)
{
    size_t num_failed = 0;
    for (size_t i = 0; i < model1.pixels_cols; i += 20)
    {
        for (size_t j = 0; j < model1.pixels_rows; j += 20)
        {
            Eigen::Vector2d p(i, j);
...
TEST(distort, DISABLED_inversemodel_image_to_3d_to_image_radial_tangential_distortion)
{
    InverseCameraModel model;
    model.focal_length_pixels = 6000;
    model.pixels_cols = 4000;
...
TEST(distort, DISABLED_inversemodel_conversion_to_model_no_distortion)
{
    ...
    EXPECT_NO_FATAL_FAILURE(verifyIsInverse(model, inverse_model, 1e-12)); // simple model inverts perfectly
    EXPECT_NO_FATAL_FAILURE(verifyIsInverse(inverse_model, model, 1e-12)); // simple model inverts perfectly
}
