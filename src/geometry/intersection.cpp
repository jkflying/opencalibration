...
std::pair<Eigen::Vector3d, double> rayIntersection(const ray_d &r1, const ray_d &r2)
{
    ...
    const double n1dn1 = r1.dir.dot(r1.dir);
    const double n1dn2 = r1.dir.dot(r2.dir);
    const double n2dn2 = r2.dir.dot(r2.dir);

    auto sqr = [](double d) { return d * d; };
    const double scale_denom = n1dn1 * n2dn2 - sqr(n1dn2);

    if (std::abs(scale_denom) > 1e-9)
    {
        const Eigen::Vector3d offset = (r1.offset - r2.offset);
        const double offsetdn1 = offset.dot(r1.dir);
        const double offsetdn2 = offset.dot(r2.dir);

        const double t = (n1dn2 * offsetdn2 - n2dn2 * offsetdn1) / scale_denom;
        const double s = (n1dn1 * offsetdn2 - n1dn2 * offsetdn1) / scale_denom;

        const Eigen::Vector3d p1 = (r1.offset + t * r1.dir), p2 = (r2.offset + s * r2.dir);
        res = 0.5 * (p1 + p2);
